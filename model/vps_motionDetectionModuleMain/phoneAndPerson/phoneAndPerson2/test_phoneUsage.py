...
def DarknetConv(x, filters, kernel_size, strides=1, batch_norm=True):
    # single darknet convolutional layer
    #Image padding
    if strides == 1:
        padding = 'same'
    else:
...
def weights_download(out='models/yolov3.weights'):
    _ = wget.download('https://pjreddie.com/media/files/yolov3.weights', out='yolov3weights.weights')
    
# weights_download() # to download weights
# yolo = YoloV3()
# load_darknet_weights(yolo, 'C:\\Users\\shamu\\OneDrive\\Desktop\\aankh\\model\\vps_motionDetectionMain\\phoneAndPerson\\phoneAndPerson\\yolov3weights.weights')
